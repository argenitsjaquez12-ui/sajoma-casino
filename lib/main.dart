...
class _CasinoState extends State<Casino> {
    ...
    showGame(title, (close) => StatefulBuilder(builder: (dialogContext, update) {
      Widget tile(String value, int index) {
        ...
        return AnimatedSwitcher(
          ...
          child: Container(
            key: ValueKey('$value-$spinNo-$index'), height: 76, margin: const EdgeInsets.all(3), alignment: Alignment.center,
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: accent.withOpacity(.55), blurRadius: 9)],
            ),
      ...
      void spin() {
        ...
        void tick() {
          ...
          update(() { grid = finalGrid; busy = false; spinNo++; });
          final base = payout();
          final reward = base > 0 ? min(base * max(1, ways ~/ 243), bet * 25) : -bet;
        ...
        child: Column(children: [
          Container(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 5),
              const Text('ANIMALES • A K Q J 10 • WILD • SCATTER • BONUS', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w800, fontSize: 11)),
              const SizedBox(height: 8),
              Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
  ...
  void casinoMiniGame(String title) {
    ...
    showGame(title, (close) => StatefulBuilder(builder: (dialogContext, update) {
      return SingleChildScrollView(
        child: Column(children: [
          AnimatedGameVisual(key: ValueKey(animationTick), title: title, active: busy),
          const SizedBox(height: 12),
