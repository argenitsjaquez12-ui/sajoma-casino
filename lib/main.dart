  ...
  void slots() {
    ...
    showGame('SAJOMA SLOTS', (close) {
      ...
      int symbolValue(String x) {
        const values = {
          'A': 220, 'K': 200, 'Q': 180, 'J': 160, '10': 140,
          '🦁': 900, '🐺': 800, '🐯': 760, '🦅': 720, '🐘': 680,
          '🐆': 640, '🐍': 600, '🐊': 560,
          '💎': 500, '👑': 450, '⭐': 350, '💰': 300, '🔔': 220, '🍀': 180,
        };
        return values[x] ?? 0;
      }
      ...
      return StatefulBuilder(builder: (dialogContext, update) {
        Widget cell(String value) {
          final special = value == 'WILD' || value == 'SCATTER' || value == 'BONUS';
          final colors = <Color>[
            Colors.redAccent, Colors.orangeAccent, Colors.amberAccent,
            Colors.greenAccent, Colors.cyanAccent, Colors.blueAccent,
            Colors.indigoAccent, Colors.purpleAccent, Colors.pinkAccent,
          ];
          final accent = special
              ? (value == 'WILD' ? Colors.cyanAccent : value == 'SCATTER' ? Colors.pinkAccent : Colors.amberAccent)
              : colors[value.hashCode.abs() % colors.length];
          return Expanded(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [accent.withOpacity(.95), Colors.black87, accent.withOpacity(.28)],
                    begin: Alignment.topLeft,
                    end: Alignment.bottomRight,
                  ),
                ...
                child: Text(
                  ...
                  style: TextStyle(
                    color: Colors.white,
                    fontSize: value.length > 6 ? 9 : (value.length > 3 ? 15 : 25),
                    fontWeight: FontWeight.w900,
          ...
          child: Column(children: [
            Container(
              ...
              child: const Column(children: [
                ...
                SizedBox(height: 3),
                Text('ANIMALES • A-K-Q-J-10 • WILD • SCATTER • BONUS • FREE SPINS', textAlign: TextAlign.center, style: TextStyle(color: Colors.cyanAccent, fontWeight: FontWeight.w800)),
              ]),
            ),
            const SizedBox(height: 10),
            gameFelt(
            ...
            neonButton(busy ? 'GIRANDO...' : 'GIRAR • $bet', busy ? null : () {
              ...
              void animateSpin() {
                ...
                if (spinStep < 8) {
                  Future.delayed(const Duration(milliseconds: 90), animateSpin);
                } else {
