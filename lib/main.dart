...
class _CasinoState extends State<Casino> {
  ...
  void keno() { if (!canBet(100)) return; final picks = <int>{}; showGame('KENO', (close) => StatefulBuilder(builder: (ctx, up) => Column(mainAxisSize: MainAxisSize.min, children: [Wrap(spacing: 5, runSpacing: 5, children: List.generate(40, (i) { final n=i+1; return FilterChip(label: Text('$n'), selected: picks.contains(n), onSelected: picks.length >= 10 && !picks.contains(n) ? null : (v) { up(() { if(v) picks.add(n); else picks.remove(n); }); }); })), const SizedBox(height: 12), Text('${picks.length}/10 seleccionados'), FilledButton(onPressed: picks.length != 10 ? null : () { final draw=<int>{}; while(draw.length<10) draw.add(1+rng.nextInt(40)); final hits=picks.intersection(draw).length; final reward=[-100,-100,-100,50,100,200,500,1000,2500,5000,10000][hits]; move(reward, 'Keno $hits aciertos ${reward >= 0 ? '+' : ''}$reward', win: reward>0); close(); showResult('KENO', 'Numeros sorteados: ${draw.toList()..sort()}\nAciertos: $hits', reward); }, child: const Text('SORTEAR'))])); })); }
