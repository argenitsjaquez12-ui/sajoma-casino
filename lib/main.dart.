...
class _CasinoState extends State<Casino> {
  ...
  void casinoMiniGame(String title) {
    if (!canBet(100)) return;
    showGame(title, (close) => Column(children: [
      Text('$title - juego virtual'),
      const SizedBox(height: 10),
      const Text('Juega con monedas virtuales. No hay dinero real ni retiros.'),
      const SizedBox(height: 14),
      FilledButton(
        onPressed: () {
          ...
          move(
            reward,
            '$title: ${reward >= 0 ? '+' : ''}$reward',
            win: reward > 0,
          );
          close();
          showResult(
            title,
            reward > 0 ? 'Ganaste monedas virtuales.' : 'Esta ronda no gano.',
            reward,
          );
          if (reward < 0) offerCoinsAfterLoss();
        },
        child: const Text('JUGAR - 100 COINS'),
      ),
    ]));
  }
